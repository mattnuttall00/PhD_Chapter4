...
ggplot(ten_rep_12_summary, aes(x=User_budget, y=Pop_size))+
  geom_line()+
  geom_line(aes(x=Manager_budget))
...
ten_rep_13_simold <- gmse_apply(
  ...
  user_budget = ub, # total budget of each stakeholder for performing actions
  usr_budget_rng = ubr, # 10% of ub
  manager_budget = 2000, # Manager has static budget
  manage_target = 125000, # target resource abundance (same as starting value)
  RESOURCE_ini = 125000, # initial abundance of resources - 50 trees per cell
  culling = TRUE, # culling is only option
  tend_crops = FALSE, # is tending crops on landscape allowed. if TRUE, user can increase yield each time step
...
for(time_step in 1:40){
  ...
  ten_rep_13[time_step, 1] <- time_step
  ten_rep_13[time_step, 2] <- sim_new$basic_output$resource_results[1]
  ten_rep_13[time_step, 3] <- sim_new$basic_output$observation_results[1]
  ten_rep_13[time_step, 4] <- sim_new$basic_output$manager_results[3]
  ten_rep_13[time_step, 5] <- sum(sim_new$basic_output$user_results[,3])
  ten_rep_13[time_step, 6] <- ub
  
  ten_rep_13_simold <- sim_new
  ub <- ub + 100
...
# I am going to increase the size of the landscape, set land_ownership=TRUE, and also allocate some public land.Private land will make up 40% and public land 60%
...
# load data
ten_rep_20_summary <- read.csv("outputs/Land_tenure/ten_rep_17-20/ten_rep_20_summary.csv")
...
# as ten_rep_20 but I am increasing the number of villages (stakeholders) to 16

system.time(ten_rep_21 <- gmse(
  time_max = 40,
  land_dim_1 = 200,
  land_dim_2 = 200, # landscape is 40,000ha or 400km2
  res_movement = 0, # trees don't move 
...
ten_rep_21_summary <- as.data.frame(gmse_table(ten_rep_21))
write.csv(ten_rep_21_summary, file="outputs/Land_tenure/ten_rep_17-20/ten_rep_21_summary.csv")

# load data
ten_rep_21_summary <- read.csv("outputs/Land_tenure/ten_rep_17-20/ten_rep_21_summary.csv")

# plots
time_cost_p21 <- ggplot(ten_rep_21_summary, aes(x=time_step,y=cost_culling))+
  geom_line()+
  theme_classic()

time_cull_p21 <- ggplot(ten_rep_21_summary, aes(x=time_step, y=act_culling))+
  geom_line()+
  theme_classic()

time_res_p21 <- ggplot(ten_rep_21_summary, aes(x=time_step, y=resources))+
  geom_line()+
  ylim(0,2000000)+
  theme_classic()

time_yield_p21 <- ggplot(ten_rep_21_summary, aes(x=time_step, y=crop_yield))+
  geom_line()+
  theme_classic()


time_cost_p21 + time_cull_p21 + time_res_p21 + time_yield_p21 
