...
# change all negative MB value to 10
r_waves_traj <- ifelse(r_waves_traj <10, 10, r_waves_traj)
...
Scen5_summary <- data.frame(Scen5)

filename <- paste("Scen5_summary_rep_", ii, ".csv", sep = "");
write.csv(Scen5_summary, file=filename, row.names = FALSE)
